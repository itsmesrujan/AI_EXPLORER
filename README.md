__AI-Powered AI Concept Explorer - A Desktop Based Application__

_[More detail specification including code/technical documentation is in progress]_

__Instructions to run__
1. Download the project.
2. Install the supporting packages as far as your operating system is concerned.
    - For instance, I have installed these packages using `pip3` command as shown below.
    - `pip3 install PySide6` ..
3. Navigate to the project folder of AI_EXPLORER.
4. Launch the application from your terminal with the command below.
    - `python3 main.py`
__Note:__ More and detailed instructions are in progress.

_Tested platforms_
IOS

_Supporting packages(so far):_
|   Package Name    |   Short Description                                           |   pip3 Command To Install     |
|-------------------|---------------------------------------------------------------|-------------------------------|
|   PySide6         | Provides access to the complete Qt 6.0+ framework             | `pip3 install PySide6`        |
|   matplotlib      | Creating static, animated, and interactive visualizations     | `pip3 install matplotlib`     |
|   scikit-learn    | Supports machine learning demonstrations                      | `pip3 install scikit-learn`   |
|   torch           | Supports deep neural networks                                 | `pip3 install torch`          |
|   nltk            | Natural Language Toolkit: for natural language processing     | `pip3 install nltk`           |
|   cv2             | Simplifies computer vision tasks                              | `pip3 install cv`             |

For more information on packages, refer to the link below.
https://pypi.org/